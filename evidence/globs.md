...
Not determined: whether the same `[] -> omitted` normalization also applies on the `manifest.json` `content_scripts` key path (as opposed to the `userScripts` runtime API). `Extension.sys.mjs:2159` reads `includeGlobs: options.include_globs` directly, with no `|| []`/null-coalescing visible, unlike the sibling `excludeMatches: options.exclude_matches || []` on the line above it. I did not trace this value all the way through `ExtensionContent.sys.mjs:1470` (`new WebExtensionContentScript(policy, options)`) to confirm whether an explicit `"include_globs": []` in a manifest ends up null or an explicit empty array at the WebIDL boundary. To resolve, one would need to either find a test with `"include_globs": []` in a manifest's `content_scripts` entry, or trace the structured-clone/IPC path from `Extension.sys.mjs` to the WebIDL dictionary construction.
...
| Empty glob list | no restriction | no restriction (confirmed for `userScripts` API; not determined for manifest `content_scripts`) | n/a |
...
## What would resolve the "not determined" item
A manifest fixture with `"content_scripts": [{"matches": [...], "include_globs": []}]` loaded end to end in a Gecko debug build (or an existing xpcshell test covering that exact case) would show whether the static-manifest path normalizes `[]` to "omitted" the same way `ext-userScripts.js` documents doing for the runtime API. This requires either finding an existing test that was not located during this search, or building/running Gecko, which is out of scope for this read-only task.
